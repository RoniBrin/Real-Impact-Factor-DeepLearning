"""
main_vgae.py - RIF pipeline using VGAE embeddings.
"""
...
RESULTS_CSV  = "/content/drive/MyDrive/RIF/rif_results_vgae.csv"
RESULTS_XLSX = "/content/drive/MyDrive/RIF/rif_results_vgae.xlsx"
...
def train_vgae_on_graph(pyg_data):
    """
    Trains VGAE on OpenAlex citation graph.
    ...
    print(f"\nStarting VGAE training for {EPOCHS} epochs...")
...
def run_perturbation_vgae(pyg_data, model):
    """
    Runs perturbation loop using VGAE embeddings.
...
def save_results(results):
    ...
    with pd.ExcelWriter(RESULTS_XLSX, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="RIF Results VGAE")
        ws = writer.sheets["RIF Results VGAE"]
...
if __name__ == "__main__":
    ...
    for target_year in range(YEAR_START, YEAR_END + 1):
        print(f"\n{'='*60}")
        print(f"  TARGET YEAR: {target_year} — VGAE")
        print(f"{'='*60}")
        ...
        # Step 3: train VGAE
        model = train_vgae_on_graph(pyg_data)

        # Step 4: perturbation + stability
        stability_scores_int, threshold = run_perturbation_vgae(
            pyg_data, model)
